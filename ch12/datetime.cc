...
TEST_CASE("Invalid boost:Gregorian::dates throw exceptions") {
    ...
    // These two lines have nothing to do with the test, but are examples of
    // initializing dates using Boost.
    boost::gregorian::date d{ 1986, 9, 15 };
    auto d2 = boost::gregorian::from_string("1986/9/15");

    // Same with these two lines: examples of getting local day and then
    // UTC day.
